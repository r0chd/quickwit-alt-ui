    ...
    let handle_blur = move |_| {
        spawn(async move {
                ...
                && let Some(element) = active_element.dyn_ref::<web_sys::HtmlElement>()
            {
                ...
                if filtered_indexes().iter().all(|index| {
                        index.index_config.index_id != search_value()
                    })
                {
...
#[component]
pub fn IndexSelector(search_value: Signal<String>, selected_index: Signal<Option<String>>) -> Element {
    rsx! {
        div {
            display: "flex",
            flex: "0 0 260px",
