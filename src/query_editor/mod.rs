        ...
        div { display: "flex", align_items: "center", margin_right: "30px",
            label {
                ...
                input {
                    ...
                    oninput: move |e| {
                        collapse_all.set(e.value().parse().unwrap_or(false));
                    },
                }
                "Collapse all"
            }
            ...
            button {
                ...
                onclick: move |_| {
                    if let Some(selected_index) = selected_index.read().clone() {
                        ...
                        spawn(async move {
                            let results = QuickwitApi::query(&selected_index, &query, max_hits(), "")
                                .await
                                .unwrap();
...
#[component]
pub fn DateRangeButton() -> Element {
    rsx! {
        button {
            tabindex: "0",
            r#type: "button",
            ...
            span { text_transform: "none", "No date range" }
            span {
                overflow: "hidden",
                pointer_events: "none",
                position: "absolute",
            ...
            if let Ok(parsed_url) = url::Url::parse(&url) {
                ...
                spawn(async move {
                    if let Some(selected_index) = selected_index() {
                        query_results.set(QuickwitApi::query(&selected_index, &query(), max_hits(), "").await.unwrap());
                    }
                });
            }
        }
                ...
                if let Ok(url) = window.location().href() {
                    if let Ok(parsed_url) = url::Url::parse(&url) {
                        if let Some(query_params) = parsed_url.query() {
                            web_sys::console::log_1(&format!("⏰ Query params: {}", query_params).into());
                        }
                    }
                }
            }
